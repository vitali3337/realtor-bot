...
const SYSTEM_PROMPT = `Ты — вежливый помощник агентства недвижимости Real Invest. Общаешься с клиентами в Telegram.
Твои задачи:
...
- Собирать номер телефона для передачи менеджеру
Правила:
- Отвечай коротко, максимум 3-4 предложения
- Используй эмодзи умеренно
- Если клиент готов — попроси номер телефона
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(
    ...
    `Я помощник агентства недвижимости Real Invest 🏠\n` +
...
bot.on("message", async (msg) => {
  ...
  const quickActions = {
    "🏠 Купить недвижимость":    { type: "ПОКУПКА",   prompt: "Клиент хочет купить недвижимость. Спроси район, бюджет и количество комнат." },
    "🏷 Продать недвижимость":   { type: "ПРОДАЖА",   prompt: "Клиент хочет продать недвижимость. Спроси район, площадь, этаж и желаемую цену." },
    "🏢 Сдать недвижимость":     { type: "СДАЧА",     prompt: "Клиент хочет сдать недвижимость. Спроси район, количество комнат и желаемую аренду." },
    "🔑 Снять недвижимость":     { type: "АРЕНДА",    prompt: "Клиент хочет снять недвижимость. Спроси район, бюджет в месяц и количество комнат." },
    "🏦 Рассчитать ипотеку":     { type: "ИПОТЕКА",   prompt: "Клиент хочет рассчитать ипотеку. Спроси стоимость объекта, первоначальный взнос и срок." },
    "📄 Документы":              { type: "ДОКУМЕНТЫ", prompt: "Клиент спрашивает про документы для сделки. Расскажи кратко что нужно при покупке/продаже." },
